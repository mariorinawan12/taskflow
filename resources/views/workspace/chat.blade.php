...
@section('content')
    ...
    <div x-data="chatComponent('{{ $workspace->id }}')">

        {{-- Header --}}
        <div class="mb-5">
            <h1 class="text-xl font-bold text-white tracking-tight">Messages</h1>
            <p class="text-gray-500 text-xs mt-0.5">Communicate with your workspace team.</p>
        </div>

        {{-- Main layout --}}
        <div class="flex gap-4 h-[calc(100vh-155px)] min-h-[500px]">

            {{-- ── Sidebar ── --}}
            <aside class="w-[240px] shrink-0 bg-gray-900 border border-gray-800 rounded-2xl flex flex-col overflow-hidden">
            ...
            {{-- ── Chat panel ── --}}
            <div class="flex-1 min-w-0 bg-gray-900 border border-gray-800 rounded-2xl flex flex-col overflow-hidden">

                {{-- Top bar --}}
                <header class="h-14 border-b border-gray-800 flex items-center gap-3 px-5 shrink-0">
                    <div
