...
<div class="grid grid-cols-3 gap-5">
    @foreach($columns as $col)
        ...
        <div class="flex flex-col">
            ...
            {{-- Column Body --}}
            <div class="flex-1 space-y-2 kanban-column"
                 data-status="{{ $col['key'] }}">
                 @foreach($columnTasks as $task)
                    <a href="{{ route('tasks.show', [$workspace->slug, $project->id, $task->id]) }}"
                       ...
                       <div class="flex items-center gap-2 mt-2.5">
                            ...
                            {{-- Due date --}}
                            @if($task->due_date)
                                <span class="text-[11px] {{ $task->due_date->isPast() && $col['key'] !== 'done' ? 'text-red-400' : 'text-gray-600' }}">
    ...
    document.querySelectorAll('.kanban-column').forEach(column => {
        new Sortable(column, {
            ...
            onEnd: function (evt) {
                ...
                fetch(`/{{ $workspace->slug }}/projects/${projectId}/tasks/${taskId}/status`, {
                    method: 'PATCH',
                    headers: {
